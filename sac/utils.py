...
from torchrl.envs import (
    ...
    EnvCreator,
    ParallelEnv,
    TransformedEnv, ToTensorImage,
)
from torchrl.envs.libs.gym import set_gym_backend, GymWrapper
from torchrl.envs.transforms import InitTracker, RewardSum, StepCounter
from torchrl.envs.utils import ExplorationType, set_exploration_type
from torchrl.modules import MLP, ProbabilisticActor, ValueOperator, ConvNet, OneHotCategorical
from torchrl.modules.distributions import TanhNormal
from torchrl.objectives import SoftUpdate
from torchrl.objectives.sac import SACLoss
from torchrl.record import VideoRecorder

from nrooms import make_env
...
def env_maker(cfg, device="cpu", from_pixels=False):
    ...
    if lib in ("gym", "gymnasium"):
        with set_gym_backend(lib):
            env = GymWrapper(make_env(render_mode='rgb_array'), from_pixels=True).to(device)
            env = TransformedEnv(
                env,
                ToTensorImage()
            )
...
def make_environment(cfg, logger=None):
    ...
    if cfg.logger.video:
        trsf_clone.insert(
            0, VideoRecorder(logger, tag="rendering/test", in_keys=["image"])
        )
...
def make_sac_agent(cfg, train_env, eval_env, device):
    ...
    actor_net = nn.Sequential(actor_enc, actor_net)
    ...
    qvalue = ValueOperator(
        in_keys=in_keys,
        module=nn.Sequential(qvalue_enc, qvalue_net),
    ).to(device)
